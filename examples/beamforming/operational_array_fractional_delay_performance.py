"""運用スパースアレイ定義を使い、小数遅延固定整相の性能評価を保存するサンプル。"""
...
def main() -> None:
    ...
    if not array_definition_path.exists():
        raise FileNotFoundError("operational sparse array JSON not found. Run design_operational_sparse_array_file.py first.")
    if not filter_bank_path.exists():
        raise FileNotFoundError("fractional delay filter bank not found. Run design_fractional_delay_filter_bank.py first.")
