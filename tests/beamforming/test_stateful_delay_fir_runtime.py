...
def test_integer_delay_and_residual_fir_match_one_shot_when_split_into_blocks() -> None:
    ...
    one_delay = StatefulIntegerDelay(delays)
    one_fir = VersionedCausalFIR(taps)
    one_delayed = one_delay.process(signal)
    one = one_fir.process(one_delayed)

    split_delay = StatefulIntegerDelay(delays)
    split_fir = VersionedCausalFIR(taps)
    ...
    for start, stop in ((0, 5), (5, 17), (17, 18), (18, 41), (41, 53)):
        ...
        filtered = split_fir.process(delayed)
...
def test_first_valid_sample_includes_integer_delay_and_fir_history() -> None:
    ...
    delayed = StatefulIntegerDelay(delays).process(signal)
    filtered = VersionedCausalFIR(taps).process(delayed)
...
def test_pending_coefficients_are_latched_atomically_at_next_block_start() -> None:
    ...
    stage = VersionedCausalFIR(old_taps, version=5)

    first_data = np.array([[1.0, 2.0], [10.0, 20.0]], dtype=np.complex128)
    first = stage.process(SignalBlock(first_data, np.ones(first_data.shape, dtype=np.bool_)))
    np.testing.assert_allclose(first.data, np.array([[1.0, 2.0], [20.0, 40.0]]))
    assert stage.active_version == 5

    stage.request_update(new_taps, version=6)
    # request時点では旧係数がactiveのままで、次のprocess先頭が唯一の公開切替境界となる。
    assert stage.active_version == 5
    second_data = np.array([[5.0, 6.0, 7.0], [50.0, 60.0, 70.0]], dtype=np.complex128)
    second = stage.process(SignalBlock(second_data, np.ones(second_data.shape, dtype=np.bool_)))
