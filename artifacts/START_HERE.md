...
2. `figures/beam_response_band_integrated_*.png` だけを見る。広帯域も狭帯域も帯域加算後 target=0dB 正規化の図である。
...
- 640 Hz / 9000 Hz の off-bin 自己 null 図、入力FFT、出力FFT、診断図は成果物から除外している。狭帯域 sanity check は 128-point FFT bin に一致する tone だけを見る。
