...
class FirApproximationResult:
    ...
    Attributes:
        ...
        window_start_samples: 全channelで共有するcircular窓先頭。shapeは``[n_beam]``、単位はsample。
    """
...
def approximate_weights_with_fir(weights: ComplexArray, tap_count: int) -> FirApproximationResult:
    ...
    window_start_samples = np.empty(AZIMUTH_DEG.size, dtype=np.int64)
    for beam_index in range(AZIMUTH_DEG.size):
        ...
        # 全channelに同一窓を適用した先頭sampleを保存し、後段の波形比較でlatencyを明示する。
        window_start_samples[beam_index] = start_index
    ...
    return FirApproximationResult(reconstructed, energy_ratio, window_start_samples)
