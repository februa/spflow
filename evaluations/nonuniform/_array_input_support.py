"""非均一filterbank評価用のアレイ入力生成helperをまとめる。"""

# 非均一木構造では分割仕様とstreaming状態の組み合わせで挙動が大きく変わるため、
# 実運用に近い入出力条件を一式そろえて評価・書き出しできるようにする。
    ...
    *,
    ...
    if np.asarray(shading_table).shape != expected_shape:
        raise ValueError(
            ...
            'Use evaluations/nonuniform/generate_array_inputs.py to regenerate matching arrays.'
        )
...
def save_array_inputs(output_dir: str | Path, design: BandwiseArrayDesign, *, fs_hz: float, sound_speed: float) -> None:
    """array designを評価再利用向けのndarray群として保存する。"""
