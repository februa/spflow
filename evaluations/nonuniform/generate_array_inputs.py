"""非均一filterbank評価用の受波器位置・シェーディング配列を生成する。"""

# 非均一木構造では分割仕様とstreaming状態の組み合わせで挙動が大きく変わるため、
# 評価条件に対応する入出力を一式そろえて再生成できるようにする。
...
from evaluations.nonuniform._array_input_support import build_default_array_design, save_array_inputs


def build_parser() -> argparse.ArgumentParser:
    """CLI 引数定義を返す。"""
    parser = argparse.ArgumentParser(
        description='非均一ビームフォーミング評価用のarray ndarrayを生成する。',
    )
