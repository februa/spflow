...
方位別時間切り出し共分散を使う2方式は、現行の固定整相に対する差分として評価する。正式な方式比較は次の3方式とし、すべてのscenarioで同じ入力波形、同じ待受beam軸、同じ解析周波数軸、同じchannel shadingを使う。
...
artifacts/beamforming/direction_cut_mvdr_method_comparison/review_pack/
  ...
  figures/<scenario_id>/
    ...
    fraz_panel.png
    fraz_delta_direct_vs_fixed.png
    fraz_delta_integer_delay_vs_fixed.png
    input_spectrum.png
...
![3方式BL](../../artifacts/beamforming/direction_cut_mvdr_method_comparison/review_pack/figures/low_broadband_endfire_0deg_interferer_60deg/bl_overlay.png)

BLは待受beam方位軸である。固定整相はtarget 0 degとinterferer 60 degの両方にpeakを保った。直接MVDRと整数遅延前段MVDRも両sourceのpeakを保ち、静的モデル上では2つの曲線が数値誤差内で一致した。

![3方式FRAZ](../../artifacts/beamforming/direction_cut_mvdr_method_comparison/review_pack/figures/low_broadband_endfire_0deg_interferer_60deg/fraz_panel.png)

FRAZはshape`[n_beam,n_frequency]`のper-bin RMS levelである。固定整相は40--120 Hzでtargetとinterfererの広い裾を持つ。2つのMVDRは0 degと60 degのridgeを保ちつつ、その他の待受方位を強く抑圧した。この差が帯域全体で生じているかはFRAZ差分図と周波数ごとの数値指標で確認する。

### 8.3 周波数スペクトル

![3方式出力スペクトル](../../artifacts/beamforming/direction_cut_mvdr_method_comparison/review_pack/figures/low_broadband_endfire_0deg_interferer_60deg/output_spectrum_overlay.png)

スペクトルはtarget最近待受beamのper-bin RMS levelであり、帯域積分RMSではない。現在の静的モデルでは2つのMVDR出力が一致する。固定整相との差は周波数ごとに表示したが、これはまだ時間波形、FIR境界、weight更新を含まない。
