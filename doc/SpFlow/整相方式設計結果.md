...
コード上では`run_evaluation`を高水準の処理順として残し、MATLAB係数読込、alias事前予測、scene生成、T2a重み設計、review pack生成を上から順に記述する。波形完全性、block境界指標、波形PNGは`scene_renderer_t2a_waveform_reporting.py`へ分離し、信号生成・T2a設計・逐次適用の本体と混在させない。CSV、NPZ、BL/FRAZ/FLをまとめる長い評価処理は`_evaluate_and_write_review_pack`へ集約し、`main`から本質的な処理順を追う際に詳細を展開しなくてよい構造とする。
...
各blockでは`process_runtime_block_with_flow`にFlow接続を一まとめにし、分岐前、方式分岐、適用後、通常Pythonへの復帰を名前付き変数で表す。

```text
input_block_flow = Flow.from_value(runtime_block)
method_branch_flow = Flow.many([fixed_branch, t2a_mvdr_branch, t2a_ebae_branch])
...
T2aの完成係数はstreaming開始前に設計され、各`StreamingBeamBranch`へ保持される。このためframeごとの信号経路と係数更新経路は存在せず、信号・係数経路のzip合流は行わない。Flowは一つの入力blockを複数方式branchへ適用する分岐だけを担当し、rate、DAG、整数delay履歴、FIR履歴、係数版を持たない。block反復と時刻位置への収集は通常の`for`文で明示する。
...
実運用MATLAB係数では`--write-example-coefficients`を付けず、2つのraw pathと実際の周波数間隔を指定する。

#### 6.7.7 T2a-EBAE単独実行

`evaluations/beamforming/scene_renderer_t2a_ebae_streaming.py`は、共通実装へ`method_ids=("t2a_ebae",)`を明示する単独CLIである。T共分散、EBAE完成重み、残差FIR、Flow branch、BL/FRAZ/FLはT2a-EBAEだけ生成する。`fixed_baseline`と`t2a_mvdr`の独立した重み設計、Flow branch、CSV行、NPZ配列、図系列は生成しない。方式ごとの数式を別ファイルへ複製せず、3方式版と単独版でMATLAB raw契約、遅延符号、shading、正規化、valid maskを同じ実装に固定する。
...
波形評価の計算と描画は`evaluations/beamforming/scene_renderer_t2a_waveform_reporting.py`へ分離する。このmoduleは完成済み波形だけを入力とし、scene生成、T2a重み設計、FIR化、Flow逐次処理、MATLAB係数読込を責務に含めない。
