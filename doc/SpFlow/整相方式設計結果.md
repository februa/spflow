...
#### 6.7.4 通常Pythonによる方式分岐

`run_streaming_beam_branches`はblock反復と方式分岐を通常のPython制御構文で表す。

```text
for each input block:
    runtime_block = RuntimeBlock(start_sample, channel_samples)
...
T2aの完成係数はstreaming開始前に設計され、各`StreamingBeamBranch`へ保持される。このためframeごとの信号経路と係数更新経路は存在せず、信号・係数経路のzip合流も行わない。各branchは入力blockごとに必ず一つの完成`BeamBlock`を返し、0個・複数個の出力や処理レート差を扱わない。この箇所で`Flow`を使っても逐次処理上の面倒は減らず、入力Flowの中へ方式Flowを渡す入れ子が分岐を読みにくくするため採用しない。整数delay履歴とFIR履歴は各branchが保持し、block反復、方式分岐、時刻位置への収集は通常の`for`文で明示する。
