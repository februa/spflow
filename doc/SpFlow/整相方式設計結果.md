...
整数遅延前段MVDRは、前段の整数遅延に合わせて共分散とsteeringのchannel位相を補正した結果、直接MVDRと同じbeam patternになった。共分散固有値は数値誤差内で一致し、target複素出力の絶対差は`1.12e-16`だった。したがって、offline解析では整数遅延前段に整相性能上の優位性は見られない。次に調べるべきは、実時系列でのbuffer境界、波形、latency、演算量の差である。
