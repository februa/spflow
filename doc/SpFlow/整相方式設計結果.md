...
低周波でS1/T1の元入力座標直接FIRと、S2a/T2aの整数遅延分離・残差FIRに必要なtap数を比較した。評価条件は150 degの単一信号、8 channel・素子間隔6 m・開口42 m、標本化周波数8192 Hz、4096点DFT、周波数間隔2 Hzとした。tap数は32、128、256、512、1024である。
...
評価結果を読む前に、アレイ配置と周波数から予測できるグレーティングローブを計算する。ULAでは`d(cos(theta_g)-cos(theta_0))=m lambda`を満たす`theta_g`が信号方位と同じセンサ間位相を与える。150 Hzでは波長10 m、素子間隔6 m、信号方位150 degから`theta_g=36.81 deg`となる。64 Hzでは可視領域内に解がない。64--256 Hzでは約0--83.6 degに周波数依存のグレーティング領域を持つ。この既知領域をFIR打切りによる偽ピークと混同しない。
...
|---|---:|---|---|
| 64 Hz狭帯域 | 32 | -12.02 dB、110 deg | 0.00 dB、150 deg |
| 64 Hz狭帯域 | 128 | -4.08 dB、140 deg | 0.00 dB、150 deg |
| 64 Hz狭帯域 | 256以上 | 0.00 dB、150 deg | 0.00 dB、150 deg |
| 150 Hz狭帯域 | 32 | -12.09 dB、160 deg | 0.00 dB、150 deg |
| 150 Hz狭帯域 | 128 | -4.09 dB、40 deg（理論GL 36.81 degの最近傍beam） | 0.00 dB、150 deg |
| 150 Hz狭帯域 | 256以上 | 0.00 dB、150 deg | 0.00 dB、150 deg |
| 64--256 Hz広帯域 | 32 | 約-12.07 dB、ピーク誤りあり | 0.00 dB、150 deg |
| 64--256 Hz広帯域 | 128 | 約-4.07 dB、150 deg | 0.00 dB、150 deg |
| 64--256 Hz広帯域 | 256以上 | 0.00 dB、150 deg | 0.00 dB、150 deg |

S1/T1は32・128 tapで整数遅延を含むインパルス応答支持を収容できず、目標感度低下または最大ピーク移動が生じた。150 Hz・128 tapで40 degが最大になる現象は、FIRが新しい40 deg偽像を生成したのではない。配置から事前予測される36.81 degのグレーティングローブが残る一方、FIR打切りにより真の150 deg応答が-4.09 dBへ低下したため、既存のグレーティングローブが最大へ入れ替わった結果である。256 tap以上では目標感度とピーク方位が回復したが、グレーティングローブ自体が方式的に解消したことを意味しない。S2a/T2aは整数遅延を前段へ分離するため、3信号条件のすべてで32 tapから目標方位150 degと0 dB応答を維持した。

ただし「目標方位が0 dB」はBL全形状がfull-DFT基準と一致することを意味しない。広帯域BLのRMS再構成誤差は1024 tapでもEBAE約0.54 dB、MVDR約1.32 dBであり、512 tapではそれぞれ約1.23 dB、2.53 dBであった。目標感度だけなら256 tapで回復する一方、BL形状まで忠実に再現する場合は512--1024 tapが候補になる。
...
64 Hzでは波長が150 Hzより長く、同じ42 m開口に対する電気的開口が小さくなる。そのため主ローブが広く、150 Hz図と並べたときに方位方向の変化が緩やかに見えることは物理的に自然である。一方、32/128 tapで最大ピーク自体が110/140 degへ移る現象は主ローブ幅だけでは説明できず、S1/T1のFIR打切り誤差である。
