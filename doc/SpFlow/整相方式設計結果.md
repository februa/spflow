...
| `coarse_covariance_direct_mvdr` | 整数遅延なしで、同一時間blockの粗い共分散からMVDRを直接設計 | 粗い分析幅の破綻を観測する参照方式。旧S0 |
| `integer_delay_then_mvdr` | S0共分散と重みを整数遅延後のchannel位相基準へ回転して適用 | S0と同じ完成重みを整数遅延後入力へ適用する座標変換方式。旧S1 |
| `direction_cut_direct_mvdr` | 切り出し時刻差の位相補正後の方位別共分散から設計したMVDR重みを元入力へ直接適用 | 整数遅延bufferを持たない候補 |
| `direction_cut_integer_delay_mvdr` | 同じ完成共分散から設計し、整数遅延後のchannel位相に合わせたMVDR | 整数遅延を前段に残す候補 |
...
### 8.0 `integer_delay_then_mvdr`実装不整合による結果の訂正

旧S1の意図は、S0共分散、steering、重みを整数遅延後入力のchannel位相基準へ回転して適用することである。共分散を再推定したり、方位別時間切り出し共分散へ置き換えたりする方式ではない。

しかし、本章の成果物を生成した`evaluations/beamforming/direction_cut_mvdr_spatial_spectral_review.py`では、`integer_delay_then_mvdr`用の共分散としてS0の`coarse_covariance`を使わず、`_direction_cut_covariance(...)`を代入してから整数遅延位相を与えていた。このため、本章の`integer_delay_then_mvdr`結果は正しいS1ではなく、実質的にT系共分散を整数遅延後座標へ回転した結果である。

したがって、以下の8.2節に記録された「`integer_delay_then_mvdr`が方位別共分散の2方式と一致した」という結果は、S1がT1/T2へ近づくことを示す証拠として無効である。正しいS1は次で定義する。

\[
R_{S1}=D R_{S0}D^H,\qquad a_{S1}=Da_{S0}
\]

S1座標の重みを`v_S1`とすると、元入力座標の等価重みは`D^H v_S1`である。`D`がunitaryなchannel位相回転で、loading規約が同じなら、正しいS1は元入力座標でS0と一致し、S0共分散のcoherenceや固有値を改善しない。

この不整合を修正して5方式成果物を再生成するまで、本章のS1数値と図を正式比較へ使用しない。T1とT2の同値性、およびS0の結果は、それぞれ独立に再確認する。
...
BLは待受beam方位軸である。固定整相はtarget 0 degとinterferer 60 degの両方にpeakを保った。`coarse_covariance_direct_mvdr`は固定整相より裾を狭めたが、低周波帯で方位方向の広がりが残った。`integer_delay_then_mvdr`は、方位別共分散を使う2方式と数値誤差内で同じ曲線になった。
...
FRAZはshape`[n_beam,n_frequency]`のper-bin RMS levelである。固定整相は40--120 Hzでtargetとinterfererの広い裾を持つ。`coarse_covariance_direct_mvdr`は低周波側に広い応答が残る。`integer_delay_then_mvdr`と方位別共分散の2方式は0 degと60 degのridgeを保ちつつ、その他の待受方位を強く抑圧し、静的モデルでは3方式が一致した。これは、整数遅延後のABFが残留小数遅延整相を担える可能性を示すが、実時系列のFIR境界とweight更新を含まないため採用結論ではない。
