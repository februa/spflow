...
各blockでは次の形だけを`Flow`へ担当させる。

```text
Flow.many([fixed_branch, t2a_mvdr_branch, t2a_ebae_branch])
    -> map(branch.process, runtime_block)
    -> map(collector)
```

Flowは0個・1個・複数個の値を運ぶだけで、rate、DAG、履歴、係数版を持たない。整数delayとFIR履歴は各branchの通常Pythonクラスが保持し、block反復は通常の`for`文で明示する。この分離により、方式追加時もFlow subclassや外部接続定義を要求しない。
