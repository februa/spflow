...
| 指標CSV | `artifacts/beamforming/ebae_mvdr_s1_s2a_t1_t2a_fir_sweep/review_pack/scenario_summary.csv` |
| 描画配列 | `artifacts/beamforming/ebae_mvdr_s1_s2a_t1_t2a_fir_sweep/review_pack/data/low_band_long_ula_beam_center.npz` |
| FIR誤差図 | `artifacts/beamforming/ebae_mvdr_s1_s2a_t1_t2a_fir_sweep/review_pack/figures/low_band_long_ula_beam_center/fir_weight_error_sweep.png` |
| BL図 | `artifacts/beamforming/ebae_mvdr_s1_s2a_t1_t2a_fir_sweep/review_pack/figures/low_band_long_ula_beam_center/source_frequency_bl_overlay.png` |
...
強弱近接信号は強信号方位0/90/150 deg、方位間隔2/5/10/20 deg、弱信号level -6/-20/-40 dB re strong、分析幅0/16 HzをS/T共分散、EBAE MUSIC/MVDR Caponで評価した。弱信号方位誤差2 deg以下、強信号とは別のlocal peak、強弱間valleyに対するprominence 3 dB以上を可視条件とした。
...
| 分類 | 可視条件数 / 72 |
|---|---:|
| EBAE・S共分散family（S1/S2a/S2b） | 30 |
| EBAE・T共分散family（T1/T2a/T2b） | 28 |
| MVDR・S共分散family（S1/S2a/S2b） | 7 |
| MVDR・T共分散family（T1/T2a/T2b） | 7 |

全方式合算では弱信号level -6/-20/-40 dBで可視条件数が41/25/6（各96条件）、方位間隔2/5/10/20 degで6/13/20/33（各72条件）となった。強信号sidelobeと主ローブ裾により、弱信号が独立peakを作れない破綻条件が多数存在する。T共分散は単一広帯域sourceのrank回復には有効だが、強弱近接sourceの可視性を無条件に保証しない。

この弱信号可視率はFIR実現前の共分散family比較である。`S covariance family`はS1/S2a/S2b、`T covariance family`はT1/T2a/T2bに共通する方位推定結果を表し、1方式と2方式の区別を省略した表記ではない。

![弱信号可視率](../../artifacts/beamforming/alignment_weak_source_visibility_sweep/review_pack/figures/weak_source_visibility_heatmap.png)
...
| 強弱信号CSV | `artifacts/beamforming/alignment_weak_source_visibility_sweep/review_pack/scenario_summary.csv` |
...
18章の完成周波数重み相当の確認を、実整数delay buffer、有限長残差FIR、block逐次処理まで拡張した。64 ch、6.25 m間隔、393.75 m開口の長大ULAへ、0 degおよび180 degから40--88 Hzの実時間帯域制限乱数信号を入力した。`fs=1024 Hz`、分析`NFFT=16`、分析幅64 Hz、非overlap snapshot 4096個、N/E AICの`L=M^2=4096`、待受方位0--180 degを2 deg刻みとする。tapは32/128/256/512/1024/2048である。
...
とする。`d_i=tau_i-min(tau)`では符号が反転し、endfire遅延を二重化するため使用しない。初回有効sampleは`max(d_i)+L_FIR-1`であり、32/128/256/512/1024/2048 tapでは299/395/523/779/1291/2315 sampleとなった。全方式でこの過渡以前を評価対象外とする。
...
| 256 | 0.983 | 0.74 dB | -0.34/-0.35 dB | 0.995/0.995 |
| 512 | 0.992 | 0.69 dB | -0.41/-0.37 dB | 0.997/0.997 |
| 1024 | 0.996 | 0.40 dB | -0.45/-0.46 dB | 0.998/0.998 |
| 2048 | 0.998 | 0.17 dB | +0.01/-0.01 dB | 0.9986/0.9987 |

本基準ではT2aの最短合格tapは256である。2048 tapは矩形帯域端を含む最大振幅誤差が約2.50 dB残るが、これは理想応答の不連続点に対する有限FIR近似である。帯域内部、帯域積分level、波形は収束しているため、端点単独を不合格理由にしない。

### 19.5 D段: 実時間target/noise/mixedとblock境界

| source | 方式 | 256 tap target level | 波形相関 | 出力SNR | 判定 |
|---:|---|---:|---:|---:|---|
| 0 deg | S2a | -0.86 dB | 0.995 | 27.69 dB | 真値beam波形は保持、MUSIC誤対応 |
| 0 deg | T2a | -0.34 dB | 0.995 | 48.05 dB | 方位・波形・SNR成立 |
| 180 deg | S2a | -0.86 dB | 0.995 | 27.38 dB | 真値beam波形は保持、MUSIC誤対応 |
| 180 deg | T2a | -0.35 dB | 0.995 | 47.20 dB | 方位・波形・SNR成立 |
...
全24条件でNaN/infはなく、257 sample block処理と一括処理の最大絶対差は0であった。target+noise出力とtarget-only+noise-onlyの最大RMS差は`3.62e-7`であった。係数更新回帰試験でも、pending係数は次block先頭でatomicに切り替わり、block内混在はない。

![有限BLと物理noise floor](../../artifacts/beamforming/formal_s2a_t2a_endfire/review_pack/bl_target_noise_components.png)

target-only実線とnoise-only点線を同じ`dB re input RMS`で描き、全条件の有限値から共通表示範囲を決めた。target-onlyの深いnullとnoise-onlyの物理noise floorを混同しない。
...
この条件では、T2aは256 tap以上で真のendfire方位、目標level、波形、SNR、block連続性を維持し、S2aより有効である。成立根拠はT共分散による同一波面snapshot形成であり、整数遅延分離だけではない。

一方、T2aのAIC信号数3は真値1と一致しないため、EBAE全体の正式採用条件は満たさない。現時点の結論は「T2aは広帯域endfireの方位/BL/runtime実現に限定して成立」である。AIC過推定、異なるSNR・積分時間、複数信号、150 deg、帯域端を滑らかにした実運用spectrum、係数更新時の適応過渡は残る境界試験とする。
...
| stateful runtime helper | `evaluations/beamforming/stateful_delay_fir_runtime.py` |
| 正式条件回帰試験 | `tests/beamforming/test_formal_s2a_t2a_endfire.py` |
| runtime/更新境界試験 | `tests/beamforming/test_stateful_delay_fir_runtime.py` |
