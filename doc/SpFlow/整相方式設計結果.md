...
方位別の時間切り出しと共通時間軸への位相復元は、T2a評価固有の処理から
`spflow.beamforming.direction_aligned_covariance`へ分離する。
`extract_direction_aligned_rfft_snapshots`は、因果遅延`d[b,c]`を受け、先頭ゼロ詰めを
混ぜずに同一波面区間を矩形窓・非正規化rFFTへ変換し、channel別切り出し時刻の位相を
channel平均の共通時刻へ復元した`[n_frame,n_frequency,n_ch]`を返す。位相式は
`calculate_snapshot_time_axis_restoration_phase`へ分け、既存の方位別共分散scheduleも
同じ式を使う。入力は公開境界で`float32`、位相とsnapshotは`complex64`へ正規化する。
ここまでをT共分散固有のsnapshot生成とする。

指定周波数、active channel、実際のsnapshot数`L`は方式側が通常のNumPy slicingで選び、
汎用`estimate_covariance`へ`[n_ch,L]`として渡す。`R=(1/L)XX^H`は方位固有処理ではない。
DC/Nyquistを含む各binの空間共分散にはone-sided conjugate-pair係数を掛けない。全帯域power
積分ではなく、同一bin内のchannel間統計だからである。T1からT2aへの`D R D^H`、残留steering、
MVDR/EBAE、scene条件、更新scheduleも固有snapshot部品へ含めない。この境界により、方位別の
時間・位相整合だけを再利用し、汎用共分散式をscene rendererや固有部品へ複製しない。
...
である。各周波数ではMATLAB表からactive channelを選び、その部分共分散と部分steeringだけでMVDRとEBAEを設計する。EBAEのN/E AICには実際に観測したnon-overlap snapshot数`L`を渡す。`L=M^2`は十分な積分の評価基準として記録するが、評価を開始する必須条件にはしない。`L<M^2`でも重み設計を続行し、積分不足によるAIC候補数の制限、MUSIC方位、fallback、出力劣化を成果物として保存する。完全snapshotが0個の初回更新だけは共分散が定義できないため固定CBFへfallbackし、次回更新を継続する。有限snapshotや上流誤差で非Hermitian成分が残る共分散は`(R+R^H)/2`へ射影し、Hermitian例外で評価を停止しない。実信号経路の複素shading`g`は`w^H x`規約の重み側へ`conj(g)`として反映する。shading適用後に`w^H a_res=1`へ再正規化し、正規化不能なら不完全な適応値を公開せず固定CBFへfallbackする。
