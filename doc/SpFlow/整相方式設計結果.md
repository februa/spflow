...
現在の`design_frequency_weights`は一更新境界の呼び出し内で全周波数・全beamを完成させる。
実計算時間が更新間隔を超える環境でのdeadline保証は本runtimeの成立範囲に含めず、その場合は
周波数item単位へ分けた`StepScheduler`接続を別途評価する。ただし、その拡張でも完成するまで
前回版を維持し、`CompletedWeightUpdate`以外を信号経路へ渡さない契約は変えない。
...
必要な適応重み更新間隔を指定する。更新間隔を省略した場合は1 sである。
