...
### 6.2 scenario orchestrationとして残すもの
...
1. private helperを跨いでいる残りのscenario workflowを`evaluations/beamforming`へ移す。
2. `array_design`内に残るplot/report保存を小さい設計結果と`tools/array_design`へ分ける。
3. SLC diagnosticsを`beamforming`から評価workflowへ移す。
4. `spflow.beamforming.__all__`と`spflow.__all__`から旧facadeとscenario workflowを段階的に外す。
