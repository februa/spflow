...
1. 整数遅延後の多channel信号にMVDRを適用し、target波形、干渉抑圧、連続性、計算量を同時に満たせるか。
2. MVDRの前段に整数遅延を置くことが、整相結果のどの誤差要因に寄与するか。
...
比較は次の4方式とする。方式AとBが整数遅延の有無だけを分離する主比較であり、CとDは運用上の比較対象である。

| ID | 前段 | 適応処理 | 拘束steering | 目的 |
|---|---|---|---|---|
| A | なし | 直接MVDR | 物理遅延全体`a` | 基準となる周波数領域MVDR |
| B | 整数遅延 | 直接MVDR | 整数遅延適用後の残留steering`D a` | 整数遅延の実装寄与をAと比較 |
| C | 整数遅延+小数遅延 | なし | 固定整相 | target保存と波形品質の安全側基準 |
| D | 整数遅延+小数遅延 | 差分MVDR | 固定整相応答と同一規約 | 従来検討方式との比較 |

方式Bで残留steeringを全channel共通の`1`に固定してはならない。整数sampleへの丸め後には最大0.5 sample相当の相対遅延が残るため、高周波ではこの残差が無視できない。`1`拘束は別の近似方式として扱い、Bの主比較に混ぜない。

### 13.4 評価ケースと成分分解

各方式は、同じ入力波形、同じ完成済み共分散、同じdiagonal loading、同じweight更新時刻で比較する。最初から多条件sweepを行わず、次の順序で方式と実装の正しさを分離する。

1. `target-only`、空間雑音なしでAとBの等価性、歪みなし拘束、出力波形を確認する。
...
AとBの理想等価性試験では、BLだけでなく、complex出力の最大絶対誤差とRMS誤差を直接比較する。運用上の採否閾値は、この数式同値性確認と代表条件の成分分解が完了した後に固定する。BLの複合scoreやparameter sweep選別には、視覚順位との一致度を未検証の指標を使用しない。

### 13.6 検討順序と判定

実装と評価は次の順序で進める。

1. offlineの複素スペクトルでA/B等価性を確認し、符号、共役、正規化、axisを固定する。
2. 方位別時間切り出し共分散をA/Bへ接続し、完成済み同一共分散からweightを作る。
3. streamingの整数遅延bufferとMVDR適用を接続し、初回、終端、block境界、weight切替を評価する。
4. C/Dを加え、波形品質、干渉抑圧、計算量、latencyのtrade-offを比較する。
5. 代表条件で成立した後にだけ、周波数、方位、SNR、干渉方位差、積分時間、loadingのsweepへ展開する。
