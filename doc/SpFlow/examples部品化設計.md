...
`examples/beamforming/fixed_delay_and_sum.py` は、外部 renderer を使わず、解析式で単一 source の waiting-beam BL を生成する。校正対象となる BL 配列と特徴量を小さな決定論的条件で得られるため、次段の人間評価データ収集に使う基準 example とする。
...
5. 簡略化した fixed delay-and-sum example から校正用 BL 群と人間評価入力を生成する。
