...
tools/design_operational_shading_file.py
tools/design_operational_shading_for_beam_count.py
tools/design_operational_sparse_array_file.py
tools/design_time_delay_guard_table.py
...
### 11.4 2026-07-14: examples配置の責務分離完了

`examples/`に残っていた評価、sweep、diagnostics、候補探索、運用ファイル生成を責務別に再配置した。

- beamforming評価21件を`evaluations/beamforming/`へ移動した。
- filterbank評価2件を`evaluations/filterbank/`へ移動した。
- nonuniform評価本体、入力helper、MATLAB変換を`evaluations/nonuniform/`へ移動した。
- beamformingの運用ファイル生成5件とfilterbank係数探索3件を`tools/`へ移動した。
- `examples/`には外部評価環境を必要としない決定論的な公開API使用例だけを残した。

この移動では方式の数式、評価条件、成果物定義を変更していない。移動後もevaluation間のprivate
helper依存は評価支援部品の抽出候補として残るが、配置整理と同時に巨大なRuntimeへ統合しない。
...
同じ8 channel、0.25 m間隔、source 65 degの条件で、周波数を6 kHz、sampling frequencyを24 kHzへ変更した。

```text
wavelength = c/f = 1500/6000 = 0.25 m
sensor spacing = 0.25 m
d/lambda = 1.0
aperture length = 1.75 m
```

ULAの空間alias条件を方向余弦`u=cos(theta)`で表す。

```text
u_alias = u_source + m*lambda/d,  m != 0
```

source 65 degに対し、可視範囲`[-1,1]`へ入る理論alias方位は125.267 degである。成分別BL評価部品の出力は次になった。

| 特徴量 | 6 kHz結果 |
|---|---:|
| peak azimuth | 65.0 deg |
| peak level error | 約0.0 dB |
| -3 dB width | 6.0 deg |
| left/right first null | 57.0 / 73.0 deg |
| left first sidelobe | -12.798 dB re mainlobe peak at 53.0 deg |
| right first sidelobe | -12.800 dB re mainlobe peak at 76.0 deg |
| detected grating-lobe candidate | -0.013 dB re mainlobe peak at 125.0 deg |
...
- `alignment.py`と`streaming.py`は既存importを維持する薄い公開ファサードとし、処理を実装しない。
- `alignment_config.py`は整相条件の保持と境界検証だけを担う。
- `ula_propagation.py`はULAの到来遅延と周波数領域steeringだけを計算する。
- `alignment_covariance.py`は平坦帯域sourceのS/T共分散モデルだけを計算する。
- `alignment_weight_design.py`は上記部品とEBAE/MVDRを組み合わせ、完成重みを設計する。
- `alignment_coordinates.py`は方式座標の識別子と整数遅延座標変換を担う。
- `frequency_weight_fir.py`はfull DFT重みの有限FIR化だけを担う。
- `source_beam_level.py`はsource成分のband積分levelを`dB re input RMS`で計算する。
- `signal_block.py`は値と完成maskの固定結果型、`integer_delay.py`は整数sample遅延、
  `versioned_fir.py`は版番号付き因果FIRと係数公開境界をそれぞれ担う。
