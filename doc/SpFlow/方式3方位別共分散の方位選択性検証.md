...
中央密・外側疏の64ch相対配置を保ち、開口長だけを0.25--32 mへscaleした。40度、128--1024 Hz、target-only、`T=10 s`の同一条件で再評価した。
...
0.25 m開口では最小etaでも0.9315であり、「開口長が短いと全方位の相関が1に近づく」ことを再現した。現在の32 m開口では40度と90度の端間遅延差が自己相関時間目安の14.6倍、正解近傍対遠方marginが0.773である。従って、方位選択性の成立を検証する開口長としては十分である。一方、長開口は長基線のgrating lobeや副peakを生むため、「大きいほど常に良い」とは結論しない。
