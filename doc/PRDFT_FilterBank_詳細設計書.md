...
- `examples/filterbank/prototype_design_eval.py`
...
`examples/filterbank/prototype_design_eval.py` を更新し、
...
`examples/filterbank/prototype_design_eval.py` を CLI 化し、
以下の条件を外から変更できるようにした。
...
`.venv` 上で以下の例を実行した。
    ...
    python examples/filterbank/prototype_design_eval.py \
        --structure polyphase_pr \
        --n-band 32 \
        --decimation 32 \
...
`.venv` 上で `examples/beamforming/scene_renderer_mvdr_eval.py` を実行し、
以下を得た。
...
`examples/beamforming/scene_renderer_mvdr_eval.py` と回帰試験では、
以下で確認した。
...
`examples/beamforming/scene_renderer_cbf_polyphase_sweep.py` の結果は以下である。
...
`examples/beamforming/scene_renderer_mvdr_polyphase_sweep.py` の結果は以下である。
