...
入力共分散は完成値として外部から受け取る。EBAEは有限snapshotや上流の丸め誤差で残る非Hermitian成分を理由に評価を停止せず、`(R + R^H) / 2`へ射影して固有分解する。EBAEは共分散の作成方法、時間切り出し、位相整合、座標変換を規定しない。

\[
R[k]=E\{X[k]X[k]^H\}
\]

N/E AICに用いる独立snapshot数を`L=rate*T`とし、実際に共分散へ積分した観測snapshot数を渡す。

\[
L=rate\,T
\]

`rate`は独立snapshot/s、`T`は秒、`M`は当該binのactive channel数である。`L=M^2`は十分な積分を与える評価基準として残すが、実行の必須条件にはしない。`L<M^2`も許容し、AIC候補数が`min(M,L)`へ制限されることや不安定な重みを評価結果として観測する。相関したoverlap snapshotを独立snapshotとして数えてはならない。
