...
一方、整数遅延後の実信号からS1共分散を新しく推定し、そのS1共分散から残留MVDR重みを再設計する場合は、完成重み自体が変わり得る。この場合の比較は、

\[
w_{mvdr,S0/T}[k]
\stackrel{?}{=}D[k]^H v_{mvdr,S1}[k]
\]

であり、単なるFIR合成の同値性ではない。過去評価でS1とT1/T2が近かったことはこの完成重みが近かったという観測であるが、並列枝と係数焼き込みが線形的に同値であることとは別の論点として扱う。
...
を作れば、並列EBAE差分補正枝と、`q_EBAE`を小数遅延FIRへ焼き込んだ補正済み1枝は、同じ時間・周波数基準で係数を再現する限り数学的に同じである。AIC、MUSIC、`delta_i`、`beta_i`の意味も変わらない。これらの意味が変わるのは、係数の実装分解を変えたときではなく、EBAEへ入力する共分散をS0からS1またはT系へ変更したときである。
