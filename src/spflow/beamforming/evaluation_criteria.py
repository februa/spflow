...
def _build_criteria() -> tuple[BeamformingEvaluationCriterion, ...]:
    ...
    return (
        ...
        BeamformingEvaluationCriterion(
            ...
            pass_guideline="target-only で target 低下が小さく、interferer-only で漏れ込みが下がること。raw と safety fallback 後を分けること。",
...
def _build_patterns() -> tuple[BeamformingEvaluationPattern, ...]:
    ...
    return (
        ...
        BeamformingEvaluationPattern(
            ...
            title="SLC 同一周波数干渉",
            description="target と interferer が同一周波数で高相関になる厳しい条件を確認する。",
        ...
        BeamformingEvaluationPattern(
            ...
            title="SLC 異周波数干渉",
            description="時間領域 SLC L=1 が効きやすい異周波数 interferer 条件を確認する。",
