...
class Flow(Generic[InputT]):
    ...
    `map`は遅延DAGを構築せず、呼び出すたびに保持中の全項目へ即時適用する。
    同じFlowへ複数回`map`しても項目ごとの分岐実行にはならず、独立した全走査になる。
    分岐間に状態依存や実行順序がある場合は、`to_list`で通常のPythonへ戻し、
    項目ごとの制御構文で依存関係を明示する。
    ...
    def map(
        ...
        **kwargs: ArgsT.kwargs,
        ...
        callbackはこの呼び出し中に全要素へ順番に適用する。元のFlowは変更しないため、
        同じFlowへ再度`map`すると、前回の結果からではなく元の全要素を別に走査する。
