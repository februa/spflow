...
def _build_criteria() -> tuple[BeamformingEvaluationCriterion, ...]:
    ...
    return (
        ...
        BeamformingEvaluationCriterion(
            criterion_id="abf_like_non_source_suppression",
            category="Adaptive BF",
            title="ABF-like non-source sector 抑圧",
            purpose="既知 source 方位を source mask として保持し、その guard 外 non-source sector の応答包絡線が下がるかを確認する。",
            required_metrics=(
                ...
                "false_peak_count",
                "angular_robustness_min_reduction_db",
                "weight_norm_or_wng",
                "condition_number",
                "realtime_factor",
            ),
            recommended_figures=("abf_like_non_source_envelope_overlay.png", "source_mask_response_summary.png"),
            pass_guideline="採否は exact marker null ではなく、source mask 外の global peak、p95、integrated level、source-to-non-source margin、false peak count で判定する。known source peak は維持する。",
            failure_interpretation="点 null や target beam leakage だけを最適化し、non-source sector へ sidelobe を押し出している可能性が高い。source mask、guard 幅、train/test 方位ずれ、または sector 制約が不足している。",
            unit_reference="source / non-source level は RMS dB20。絶対値は dB re input RMS、処理前後差は dB re before level として基準を明記する。WNG/weight_norm、condition_number、realtime_factor は無次元比。",
        ),
        BeamformingEvaluationCriterion(
...
def _build_patterns() -> tuple[BeamformingEvaluationPattern, ...]:
    ...
    return (
        ...
        BeamformingEvaluationPattern(
            pattern_id="ABF_like_non_source_suppression",
            title="ABF-like non-source sector 抑圧",
            description="既知 source 方位を source mask とし、guard 外 non-source sector の包絡線抑圧で固定整相、SLC、LCMV/GSC、STFT/Capon を比較する。",
            required_criterion_ids=(
            ...
            recommended_criterion_ids=("fraz_btr_consistency", "input_output_level_consistency", "waveform_integrity"),
        ),
        BeamformingEvaluationPattern(
